...
class TestMissionService(unittest.TestCase):
    def _build_services(self, temp_dir: str):
        ...
        return context, race_service, unit_service, allegiance_service, mission_service, missionbook_path

    def test_create_edit_reload_missionbook(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            context, race_service, unit_service, allegiance_service, mission_service, missionbook_path = self._build_services(temp_dir)
    ...
    def test_invalid_references_are_rejected(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            _context, race_service, unit_service, allegiance_service, mission_service, _missionbook_path = self._build_services(temp_dir)
    ...
    def test_duplicate_names_get_unique_ids(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            _context, _race_service, _unit_service, _allegiance_service, mission_service, _missionbook_path = self._build_services(temp_dir)
            a = mission_service.create_mission_from_dict({"name": "Ambush"})
            b = mission_service.create_mission_from_dict({"name": "Ambush"})
